// ==================== ADMIN ACTIVITY LOG (R2 VERSION - ORIGINAL DESIGN) ====================
...
export async function handleAdminActivity(req, env, ctx, auth) {
  ...
  try {
    // Get data from R2 helper
    const { logs, total, totalPages, actions } =
      await getActivities(env, filter, days, page, ITEMS_PER_PAGE);
    ...
    const content = `
      <div style="margin-bottom: 20px;">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 10px;">
              ...
              <a href="/admin/activity/export?days=${days}&filter=${encodeURIComponent(filter)}" class="btn btn-secondary">
      ...
      <style>
          ...
          .activity-text { font-size: 0.95rem; margin-bottom: 4px; }
          ...
          .empty-state i {
              font-size: 3rem; color: #ccc; margin-bottom: 15px;
          }
      </style>
    ...
    return { content: `<div style="padding:40px;text-align:center;">Error loading activity log</div>`, title: 'Error' };
  }
}
...
function generatePagination(currentPage, totalPages, filter, days) {
  ...
  if (currentPage > 1) {
    html += `<a href="/admin/activity?page=${currentPage-1}&filter=${encodeURIComponent(filter)}&days=${days}" class="pagination-item">Prev</a>`;
  }
  ...
  if (currentPage < totalPages) {
    html += `<a href="/admin/activity?page=${currentPage+1}&filter=${encodeURIComponent(filter)}&days=${days}" class="pagination-item">Next</a>`;
  }
